...
const EMPTY_FORM: Omit<Assignment, "id"> = {
  title: "", subject: "", dueDate: "", completed: false, priority: "Medium",
};

function generateId(): string { return `a-${Date.now()}`; }
function load(): Assignment[] {
  if (typeof window === "undefined") return [];
  try { const r = localStorage.getItem("spms-assignments"); return r ? JSON.parse(r) : []; } catch { return []; }
}
function save(data: Assignment[]) { localStorage.setItem("spms-assignments", JSON.stringify(data)); }
...
export default function AssignmentsPage() {
  ...
  useEffect(() => { setAssignments(load()); setLoading(false); }, []);
  ...
  function toggle(id: string) {
    const u = assignments.map((a) => a.id === id ? { ...a, completed: !a.completed } : a);
    setAssignments(u); save(u);
  }
  function remove(id: string) { const u = assignments.filter((a) => a.id !== id); setAssignments(u); save(u); }
  function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!form.title || !form.dueDate) return;
    const u = [...assignments, { ...form, id: generateId() }];
    setAssignments(u); save(u); setForm({ ...EMPTY_FORM }); setShowForm(false);
  }
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-sm border border-indigo-100 p-6 mb-6 space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div><label className="block text-sm font-medium text-slate-600 mb-1">Title *</label>
              <input type="text" required value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400" /></div>
            <div><label className="block text-sm font-medium text-slate-600 mb-1">Subject *</label>
              <input type="text" required value={form.subject} onChange={(e) => setForm({ ...form, subject: e.target.value })} className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400" /></div>
          ...
          <button type="submit" className="bg-indigo-600 hover:bg-indigo-700 text-white font-medium px-6 py-2 rounded-lg transition-colors">Save</button>
        </form>
      )}

      <div className="flex gap-2 flex-wrap mb-6">
        {subjects.map((s) => (
          <button key={s} onClick={() => setFilter(s)} className={`px-3 py-1.5 rounded-full text-sm font-medium transition-colors ${filter === s ? "bg-indigo-600 text-white" : "bg-white border border-slate-200 text-slate-600 hover:bg-indigo-50"}`}>{s}</button>
        ))}
      </div>
      ...
      {filtered.length === 0
        ...
        : <div className="space-y-3">
            {filtered.sort((a, b) => new Date(a.dueDate).getTime() - new Date(b.dueDate).getTime()).map((a) => (
              <div key={a.id} className={`bg-white rounded-xl shadow-sm border p-4 flex items-center gap-4 ${a.completed ? "opacity-60 border-slate-100" : "border-indigo-100"}`}>
                <input type="checkbox" checked={a.completed} onChange={() => toggle(a.id)} className="h-4 w-4 cursor-pointer accent-indigo-600 flex-shrink-0" />
