...
interface SubjectSummary {
  subject: string; present: number; absent: number; late: number; total: number; percentage: number;
}
...
const load = (): AttendanceRecord[] => { try { const r = localStorage.getItem("spms-attendance"); return r ? JSON.parse(r) : []; } catch { return []; } };
const save = (d: AttendanceRecord[]) => localStorage.setItem("spms-attendance", JSON.stringify(d));
const uid = () => `att-${Date.now()}`;
...
export default function AttendancePage() {
  ...
  useEffect(() => { setRecords(load()); setReady(true); }, []);

  const summaries = getSummaries(records);
  const add = (e: React.FormEvent) => {
    e.preventDefault();
    const u = [...records, { ...form, id: uid() }];
    setRecords(u); save(u);
    setForm({ subject: "", date: new Date().toISOString().split("T")[0], status: "Present" });
    setShowForm(false);
  };
  const del = (id: string) => { const u = records.filter(r => r.id !== id); setRecords(u); save(u); };
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-slate-500 mt-1">{records.length} records · {summaries.length} subjects</p>
        </div>
      ...
      {showForm && (
        <form onSubmit={add} className="bg-white rounded-xl border border-indigo-100 p-6 mb-6">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div><label className="block text-sm font-medium text-slate-600 mb-1">Subject *</label>
              <input required value={form.subject} onChange={e => setForm({ ...form, subject: e.target.value })} className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400" placeholder="e.g. Physics" /></div>
          ...
          <button type="submit" className="mt-4 bg-indigo-600 hover:bg-indigo-700 text-white font-medium px-6 py-2 rounded-lg">Save</button>
        </form>
      )}

      <div className="flex gap-2 mb-6">
        {(["summary", "log"] as const).map(t => (
          <button key={t} onClick={() => setTab(t)} className={`px-4 py-2 rounded-lg text-sm font-medium capitalize transition-colors ${tab === t ? "bg-indigo-600 text-white" : "bg-white border border-slate-200 text-slate-600 hover:bg-indigo-50"}`}>
            {t === "summary" ? "Subject Summary" : "Full Log"}
          </button>
        ))}
      </div>

      {tab === "summary" && (summaries.length === 0
        ? <div className="text-center py-16 text-slate-400">No records yet. Mark attendance to see summaries.</div>
        : <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {summaries.sort((a, b) => a.percentage - b.percentage).map(s => (
              <div key={s.subject} className={`bg-white rounded-xl border p-5 ${s.percentage < 75 ? "border-red-200" : "border-indigo-100"}`}>
        ...
        : <div className="space-y-2">
            {[...records].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()).map(r => (
              <div key={r.id} className="bg-white rounded-xl border border-indigo-100 p-4 flex items-center gap-4">
                ...
                <button onClick={() => del(r.id)} className="text-slate-300 hover:text-red-400 text-lg">&times;</button>
              </div>
            ))}
          </div>
      )}
